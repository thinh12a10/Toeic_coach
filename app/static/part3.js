...
async function loadQuestions() {
    showLoading(loadingIndicatorEl, "Loading questions...");
    try {
        const response = await fetch("/api/part3/generate");
        if (!response.ok) {
            throw new Error(`Failed to load Part 3 questions (${response.status})`);
        }
        ...
        console.error("Failed to load Part 3 questions:", error);
        if (questionTextEl) {
            questionTextEl.innerHTML = `
...
async function evaluateAudio(audioBlob) {
    ...
    try {
        const response = await fetch("/api/part3/evaluate", {
            method: "POST",
            body: formData,
        });
